    ...
    def vypis_slozky_ve_slozce(cesta_od_rootu):
        ...
        for polozka in os.listdir(cesta_od_rootu):
            ...
            if os.path.isdir(cela_cesta):
                print(f"📁 {polozka}")
    @staticmethod
    def smaz_to(cesta_k_souboru):
        """
        Smaže soubor na zadané absolutní cestě.
        """
        if not os.path.isabs(cesta_k_souboru):
            print("❌ Chyba: Zadej absolutní cestu k souboru.")
            return
        ...
        potvrzeni = input(
            f"⚠️ Opravdu chceš smazat soubor?\n{cesta_k_souboru}\n(ano/ne): "
        ).strip().lower()
        ...
        try:
            ...
            print(f"🗑️ Soubor byl úspěšně smazán: {cesta_k_souboru}")
        except PermissionError:
            print("❌ Nemáš oprávnění smazat tento soubor.")
        except Exception as e:
